...
#list
  b-container
    b-row
      b-col.d-flex.flex-column(cols='6')
        b-link.text-white.mb-5(href='#/list')
          font-awesome-icon(:icon='["fas", "list"]')
          span.ml-2 TO-DO-LIST
        b-link.text-white(href='#/settings')
        ...
        #timer
          ...
          h2.text-danger {{ timeText }}
          h1.text-dark {{ currentText }}
          //- 目前倒數剩餘時間

      b-col#listArea(cols='6')
        //- 新增功能
        //- :state='state' 驗證狀態, 是 bootstrap-vue 的用法, 'state' 是下面 methods 裡的 state() function, 有 true false null
...
export default {
  ...
  computed: {
    ...
    list () {
      // 因為上面 :state='data.item.state' 原本沒有這個 state 做驗證, 所以用 map(), 運算完後回傳一個新陣列
  ...
  methods: {
    ...
    // (index) 是上方 @click='editlist(data.index) 傳進來的 index
    editlist (index) {
